...
function App() {
  ...
  const [trailer, setTrailer] = useState({ show: false, url: 'https://www.youtube.com/embed/hf8EYbVxtCY' })
  ...
  useEffect(() => {
    if (data.length > 0 && !search.term) {
      const urls = data.map((item: item, index: number, data: object[]) => {
      ...
      Promise.all(urls.map(item => fetch(item.streamProviderUrl))).then(resp => Promise.all(resp.map(r => r.text()))).then((streamProviders) => {
        ...
        const augmentedData = data.map((item: item, index) => (item.id === providers[index].id ? { ...item, streamProvider: providers[index].streamProvider } : item))
      ...
      Promise.all(urls.map(item => fetch(item.trailerKeyUrl))).then(resp => Promise.all(resp.map(r => r.text()))).then((trailers) => {
        ...
        const augmentedData = data.map((item: item, index) => (item.id === json[index].id ? { ...item, trailerUrl: json[index].url } : item))
      ...
      <Tabs variant='soft-rounded' colorScheme='green' isFitted>
        ...
        <TabPanels> {loading ?
          <CircularProgress isIndeterminate color='teal.300' mt={5} /> : TABS.map((item, index) => (
            <TabPanel key={index}>
              <SimpleGrid minChildWidth='190px' spacing='20px'> {data.map((item, index) => (
                <Box key={index} >
